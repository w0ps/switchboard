...
function openTagChat( event ) {
  ...
  // JF 2016-08-19 get a "unified" Need ID, so that there is a single tagchat per tag, not per need
  var UnifiedTagChatNeed = getUnifiedTagChatNeed(this._id);
  
  // console.log ("UnifiedTagChatNeed._id: "+  UnifiedTagChatNeed._id);
  //


  Meteor.call( 'updateAutogeneratedTagChatMessages', UnifiedTagChatNeed._id );


  if( isAllowed( 'separate windows' ) ) {
    var windowName = this.title;

    chatWindows[ windowName ] = window.open( '/needs/tagged/' + UnifiedTagChatNeed._id, windowName, 'height=' + constants.chatHeight + ',width=' + constants.chatWidth + ',left=' + window.innerWidth );
    return false;
  }

  var openTagConversations = Session.get( 'openTagConversations' ) && Session.get( 'openTagConversations' ).slice() || [],
      index = openTagConversations.indexOf( UnifiedTagChatNeed._id );

  if( index > -1 ) openTagConversations.splice( index, 1 );
  openTagConversations.unshift( UnifiedTagChatNeed._id );
 
  Session.set( 'openTagConversations', openTagConversations );
  Meteor.call( 'joinTagChat', UnifiedTagChatNeed._id );



  // ---- JF commented this original code out 2016-08-19
